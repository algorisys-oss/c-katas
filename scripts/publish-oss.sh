...
#   OSS_REMOTE=https://github.com/algorisys-oss/c-katas.git ./scripts/publish-oss.sh --push
#
...
OSS_REMOTE="${OSS_REMOTE:-https://github.com/algorisys-oss/c-katas.git}"
...
if [[ "$DRY_RUN" == false && -z "$OSS_REMOTE" ]]; then
    error "OSS_REMOTE is not set. Export it or pass it inline:\n  OSS_REMOTE=https://github.com/algorisys-oss/c-katas.git $0 --push"
fi
...
echo ""
info "Modules found:"
(cd "$WORK_DIR" && ls -d [0-9]*/ 2>/dev/null || echo "  (none)")
