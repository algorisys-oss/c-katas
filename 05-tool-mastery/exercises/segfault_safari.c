...
{
    /* BUG: no NULL check on ptr — crashes if ptr is NULL */
    int sum = 0;
    for (int i = 0; i < len; i++) {
        sum += ptr[i];  /* BUG: dereferences NULL pointer */
    }
    return sum;
}
 ...
 *
 * Fix: correct the base case condition.
 */
int crash2_recursive_sum(int n)
{
    if (n == -1) {  /* BUG: base case should be n <= 0, not n == -1 */
        return 0;   /* For positive n, this is never reached */
    }
...
{
    char *str = "hello world";  /* BUG: pointer to read-only literal */
    str[0] = 'H';              /* BUG: CRASH — writing to read-only memory */
 ...
 *
 * Fix: check length before copying, or use strncpy with the buffer size.
 */
int crash4_buffer_overflow(const char *input)
{
    char buf[16];  /* small buffer — only 16 bytes */
    strcpy(buf, input);  /* BUG: no length check — overflows if input > 15 chars */
 ...
 *
 * Fix: use correct bounds for each dimension.
 */
#define ROWS 3
#define COLS 5
...
int crash5_matrix_sum(void)
{
    ...
    /* Sum all elements — but bounds are SWAPPED */
    int sum = 0;
    for (int r = 0; r < COLS; r++) {  /* BUG: should be r < ROWS */
        for (int c = 0; c < ROWS; c++) {  /* BUG: should be c < COLS */
            sum += matrix[r][c];
        }
    }
    return sum;
