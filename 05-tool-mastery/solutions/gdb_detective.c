 ...
 *
 * FIX: changed i <= len to i < len
 */
int bug1_sum_array(int *arr, int len)
{
    int sum = 0;
    for (int i = 0; i < len; i++) {  /* FIX: i < len (was i <= len) */
        sum += arr[i];
    }
    return sum;
}
 ...
 * is wrong.
 *
 * FIX: initialize max to arr[0]
 */
int bug3_find_max(int *arr, int len)
{
    int max = arr[0];  /* FIX: initialized to arr[0] (was uninitialized) */
    for (int i = 0; i < len; i++) {
        if (arr[i] > max) {
            max = arr[i];
        }
 ...
 *
 * FIX: use static buffer so it survives function return
 */
const char *bug4_make_greeting(const char *name)
{
    static char buf[128];  /* FIX: static (was local — died on return) */
 ...
 *
 * FIX: changed int to long long for result and return type
 */
long long bug5_factorial(int n)  /* FIX: long long return (was int) */
{
    long long result = 1;  /* FIX: long long (was int — overflows for n >= 13) */
    for (int i = 2; i <= n; i++) {
        result *= i;
    }
    return result;
...
int main(void)
{
    ...
    printf("\nBug 5: Integer overflow\n");
    {
        ...
        long long big = bug5_factorial(15);
