...
int tokenize(char *input, char **tokens, int max_tokens)
{
     ...
     *   char buf[MAX_TOKEN_LEN] — scratch buffer for current token
     *   int buf_len = 0         — current length of token being built
     *   int count = 0           — number of tokens so far
     *   int in_quotes = 0       — are we inside double quotes?
     ...
     *
     * Step 2: Walk character by character (i = 0 to len - 1)
     *   For each input[i]:
     *
     *   a) If input[i] == '"':
     *      Toggle in_quotes. Don't add the quote to buf.
     *
     *   b) If in_quotes:
     *      Append input[i] to buf (everything is literal inside quotes).
     *
     *   c) If isspace(input[i]):
     *      If buf_len > 0, we've reached the end of a token.
     *      Null-terminate buf, strdup it into tokens[count++], reset buf_len.
     *
     *   d) If input[i] == '|':
     *      Finish current token (if any), then add "|" as its own token.
     *
     *   e) If input[i] == '>':
     *      Finish current token (if any).
     *      Check if input[i+1] == '>' — if so, emit ">>" and skip i+1.
     *      Otherwise emit ">".
     *
     *   f) If input[i] == '<':
     *      Finish current token (if any), then add "<" as its own token.
     *
     *   g) Otherwise:
     *      Append input[i] to buf.
     *
     * Step 3: After loop, if buf_len > 0, flush the last token.
     *
     * Step 4: Set tokens[count] = NULL and return count.
     */
    (void)input;
    (void)tokens;
    (void)max_tokens;
