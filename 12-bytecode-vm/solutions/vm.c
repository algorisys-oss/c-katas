...
{
    int copy_len = length > CODE_SIZE ? CODE_SIZE : length;
    memcpy(vm->code, bytecode, copy_len);
    vm->ip = 0;
    vm->sp = 0;
    vm->running = 1;
    vm->error = VM_OK;
...
/* ── Exercise 5: vm_run ───────────────────────────────────────────── */
#define MAX_CYCLES (CODE_SIZE * STACK_SIZE)
    ...
    while (vm->running && vm->error == VM_OK) {
        ...
        switch (opcode) {
            ...
            case OP_ADD: {
                int32_t b = vm_pop(vm);
                int32_t a = vm_pop(vm);
                if (vm->error == VM_OK) vm_push(vm, a + b);
                break;
            }
            ...
            case OP_DIV: {
                int32_t b = vm_pop(vm);
                int32_t a = vm_pop(vm);
                if (vm->error != VM_OK) break;
                if (b == 0) {
                    vm->error = VM_ERR_DIV_ZERO;
                    vm->running = 0;
                    break;
                }
                vm_push(vm, a / b);
            ...
            case OP_NEG: {
                ...
                if (vm->error == VM_OK) vm_push(vm, -value);
                break;
            }
            ...
            case OP_EQ: {
                int32_t b = vm_pop(vm);
                int32_t a = vm_pop(vm);
                if (vm->error == VM_OK) vm_push(vm, (a == b) ? 1 : 0);
                break;
            }
            ...
            case OP_NOT: {
                int32_t value = vm_pop(vm);
                if (vm->error == VM_OK) vm_push(vm, (!value) ? 1 : 0);
                break;
            }
            ...
            case OP_JMP: {
                int32_t target = read_int32(&vm->code[vm->ip]);
                vm->ip = target;
                break;
            }
            ...
            case OP_DUP: {
                ...
                int32_t value = vm->stack[vm->sp - 1];
                vm_push(vm, value);
                break;
            }
