...
static pool_t pool_create(size_t block_size, size_t block_count) {
    ...
    for (size_t i = 0; i < block_count; i++) {
        free_node_t *node = (free_node_t *)(p.buffer + i * block_size);
        if (i + 1 < block_count) {
            node->next = (free_node_t *)(p.buffer + (i + 1) * block_size);
        } else {
            node->next = NULL;
        }
    }
    p.free_list = (free_node_t *)p.buffer;
