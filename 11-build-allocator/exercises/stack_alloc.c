...
static stack_alloc_t stack_alloc_create(size_t capacity) {
    ...
    /* TODO:
     * 1. malloc `capacity` bytes into s.buffer.
     * 2. Set s.capacity = capacity, s.top = 0.
     * 3. Handle malloc failure (set buffer to NULL). */
...
static void *stack_alloc_push(stack_alloc_t *s, size_t size) {
    /* TODO:
     * 1. Save current top as `old_top`.
     * 2. Align top to STACK_ALIGN:
     *      aligned = (s->top + (STACK_ALIGN-1)) & ~(STACK_ALIGN-1)
     * 3. Compute header position = aligned.
     * 4. Compute data position = header position + sizeof(stack_header_t).
     * 5. Compute new_top = data position + size.
     * 6. If new_top > s->capacity, return NULL.
     * 7. Write header: ((stack_header_t*)(s->buffer + header_pos))->total_size
     *    = new_top - old_top;
     * 8. Set s->top = new_top.
     * 9. Return s->buffer + data_pos. */
...
static void stack_alloc_pop(stack_alloc_t *s, void *ptr) {
    /* TODO:
     * 1. The header is at: (stack_header_t*)((unsigned char*)ptr - sizeof(stack_header_t))
     * 2. Read header->total_size.
     * 3. Subtract total_size from s->top. */
...
static void stack_alloc_destroy(stack_alloc_t *s) {
    /* TODO: free(s->buffer), then zero the struct. */
    (void)s;
}
