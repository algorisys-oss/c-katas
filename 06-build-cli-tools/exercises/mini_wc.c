...
wc_result count_stream(FILE *fp) {
     ...
     *
     * Use fgetc(fp) to read one character at a time.
     * fgetc returns EOF when the stream ends.
     *
     * int c;
     * int in_word = 0;
...
void print_result(wc_result r, const char *filename) {
    /* TODO: Print the result in the format shown above.
     *
     * if (filename != NULL)
...
int main(int argc, char *argv[]) {
     ...
     *    - If argc > 1, open argv[1] as a file with fopen.
     *      If fopen fails, print an error to stderr and return 1.
     *    - If argc == 1, use stdin.
     *
     * 2. Call count_stream() on the input.
     *
     * 3. Call print_result() with the filename (or NULL for stdin).
     *
     * 4. Close the file if you opened one.
     *
     * 5. Return 0.
     *
     * Skeleton:
